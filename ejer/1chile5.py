...
try:
    ...
    try:
        ...
        # Fallback si no están disponibles los locales chilenos
        pass
...
def cargar_tienda1():
    ...
    try:
        ...
        # Convertir precio a numérico, importante para evitar el error de str/int
        df[col_precio] = pd.to_numeric(df[col_precio], errors='coerce')
        
        # Calcular facturación (solo precio)
        df['facturacion'] = df[col_precio].fillna(0)
        ...
        if 'costo_envio' in df.columns:
            ...
            df['costo_envio'] = df['costo_envio'].apply(lambda x: min(x, 100000))
        else:
...
def analizar_tienda_final(df):
    ...
    try:
        ...
        # Tendencia mensual - usando 'ME' en lugar de 'M' (obsoleto)
        resultados['tendencia_mensual'] = df.set_index('fecha').resample('ME')['facturacion'].sum()
        resultados['costos_mensuales'] = df.set_index('fecha').resample('ME')['costo_envio'].sum()
...
def generar_graficos_finales(df):
    ...
    try:
        ...
        plt.subplot(2, 2, 1)
        fact_mensual = df.set_index('fecha').resample('ME')['facturacion'].sum() / 1e9  # Miles de millones
        costos_mensual = df.set_index('fecha').resample('ME')['costo_envio'].sum() / 1e6  # Millones
        ...
        # Crear columnas temporales para la visualización en miles
        sample = sample.copy()
        sample['facturacion_miles'] = sample['facturacion'] / 1000
        sample['costo_envio_miles'] = sample['costo_envio'] / 1000
        
        sns.scatterplot(data=sample, x='facturacion_miles', y='costo_envio_miles', 
                        hue='categoria' if 'categoria' in df.columns else None)
        ...
        # Mostramos más información sobre el error para depurar
        import traceback
        traceback.print_exc()
